...
- (void)windowDidLoad {
    ...
    srect = NSMakeRect(10.,15.,wf.size.width-20.,118.); // !!!! 105
...
- (void) updateResultString:(NSNotification *) note {
    ...
    resultString = [resultString stringByAppendingFormat:@"\ndE/dx: CEE = %.2f, CEH = %.2f, TOTAL = %.2fMeV",theStack.dEdxCEE,theStack.dEdxCEH,theStack.dEdxCEE+theStack.dEdxCEH];
   if(CEE) {
        resultString = [resultString stringByAppendingFormat:@"\nCassette %d (No Si) Odd: %.2fX0 dE/dx = %.2fMeV;\n                      Even: %.2fX0, dE/dx = %.2fMeV",theStack.ncassette,theStack.x0Odd,theStack.dEdxOdd,theStack.x0Even,theStack.dEdxEven];
        resultString = [resultString stringByAppendingFormat:@"\nCassette thickness = %.2fmm",_stackView.layerThickness];
    } else {
        resultString = [resultString stringByAppendingFormat:@"\nOne cassette (fine) dE/dx = %.2fMeV, X0 = %.2f, λ = %.2f (without sensor)",theStack.dEdxCEHcasFine,theStack.x0CEHcasFine,theStack.lambdaCEHcasFine];
        resultString = [resultString stringByAppendingFormat:@"\nOne cassette (coarse) dE/dx = %.2fMeV, X0 = %.2f, λ = %.2f (without sensor)",theStack.dEdxCEHcasCoarse,theStack.x0CEHcasCoarse,theStack.lambdaCEHcasCoarse];
