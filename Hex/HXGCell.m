...
- (void) makePartialMods: (int) iflag {
    ...
    double xoff[6],yoff[6];
    xoff[0] = -_side*0.5; yoff[0] = - hftof; //
    xoff[1] =  _side*0.5; yoff[1] = - hftof; //      4  3
    xoff[2] =  _side;     yoff[2] = 0.0;     //    5      2
    xoff[3] =  _side*0.5; yoff[3] = hftof;   //      0  1
    xoff[4] = -_side*0.5; yoff[4] = hftof;   //
    xoff[5] = -_side;     yoff[5] = 0.0;     //
     */

    _partialColor = _cellColor;
    _partialCell = [NSBezierPath bezierPath];
    _modifiedPartial = YES;

    if(iflag == 0) { // -----------------  outside edge of LD1 (iu = 7)
        if(_edge) {
            int j = 1;
            if(_iv != 0) j = 4;
            NSPoint pnt = waferPoints[j];
            [_partialCell moveToPoint:pnt];
            if(_iv != 0) pnt.y -= 3.*hftof;
            else {
                pnt.x -= 3.*hftof*sin60;
                pnt.y -= 3.*hftof*cos60;
            }
            [_partialCell lineToPoint:pnt];
            if(_iv == 0) {
                pnt = NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5]);
            } else {
                pnt = NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1]);
            }
            [_partialCell lineToPoint:pnt];
            pnt.x += 1.5*_side*cos60;
            pnt.y += 1.5*_side*sin60;
            [_partialCell lineToPoint:pnt];
            [_partialCell closePath];
            _partialColor = [NSColor orchidPink]; // corner cell
        } else {
            NSPoint pnt = NSMakePoint(_centre.x+xoff[4]+0.5*_side*cos60, _centre.y+yoff[4]+0.5*_side*sin60);
            [_partialCell moveToPoint:pnt];
            [_partialCell lineToPoint:NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5])];
            [_partialCell lineToPoint:NSMakePoint(_centre.x+xoff[0], _centre.y+yoff[0])];
            [_partialCell lineToPoint:NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1])];
            pnt = NSMakePoint(_centre.x+xoff[2]+0.5*_side*cos60, _centre.y+yoff[2]+0.5*_side*sin60);
            [_partialCell lineToPoint:pnt];
            [_partialCell closePath];
        }
    } else if(iflag == 1) { // -----------------  outside edge of LD2 (iu = 8)
        if(_corner) {
            int j = 1;
            if(_iv != 0) j = 4;
            NSPoint pnt = waferPoints[j];
            [_partialCell moveToPoint:pnt];
            if(_iv == 0) pnt.y += 2.*hftof;
            else {
                pnt.x += 2.*hftof*sin60;
                pnt.y += 2.*hftof*cos60;
            }
            [_partialCell lineToPoint:pnt];
            if(_iv == 0) {
                pnt = NSMakePoint(_centre.x+xoff[4], _centre.y+yoff[4]);
            } else {
                pnt = NSMakePoint(_centre.x+xoff[2], _centre.y+yoff[2]);
            }
            [_partialCell lineToPoint:pnt];
            if(_iv == 0) {
                pnt = NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5]);
            ...
            NSPoint pnt = NSMakePoint(_centre.x+xoff[1],_centre.y+yoff[1]);
            [_partialCell moveToPoint:pnt];
            for(int i=2; i<6; i++) {
                pnt = NSMakePoint(_centre.x+xoff[i],_centre.y+yoff[i]);
                [_partialCell lineToPoint:pnt];
            }
            [_partialCell closePath];
        }
    } else if(iflag == 2) { // -----------------  special on a line
        NSPoint pnt = NSMakePoint(_centre.x+xoff[0], _centre.y+yoff[0]);
        [_partialCell moveToPoint:pnt];
        for(int i=1; i<4; i++) {
            pnt = NSMakePoint(_centre.x+xoff[i], _centre.y+yoff[i]);
            [_partialCell lineToPoint:pnt];
        ...
        [_partialCell closePath];
        _partialColor = [NSColor paleBlue]; // inside cell
        _notSplit = YES;
    } else if(iflag == 3) { // -----------------  (2,9) has a special shape
        NSPoint pnt = NSMakePoint(_centre.x+xoff[3], _centre.y+yoff[3]);
        [_partialCell moveToPoint:pnt];
        pnt = NSMakePoint(_centre.x+xoff[2], _centre.y+yoff[2]);
        [_partialCell lineToPoint:pnt];
        pnt = waferPoints[5];
        ...
        [_partialCell closePath];
        _partialColor = [NSColor orchidPink]; // corner cell
    } else if(iflag == 4) { // -----------------  (15,15) needs cut
        NSPoint pnt = NSMakePoint(_centre.x+xoff[0], _centre.y+yoff[0]);
        [_partialCell moveToPoint:pnt];
        pnt = NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1]);
        [_partialCell lineToPoint:pnt];
        ...
        [_partialCell lineToPoint:pnt];
        [_partialCell closePath];
    } else if(iflag == 5) { // -----------------  (14,15) needs expansion
        NSPoint pnt = NSMakePoint(_centre.x+xoff[0], _centre.y+yoff[0]);
        [_partialCell moveToPoint:pnt];
        pnt = NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1]);
        [_partialCell lineToPoint:pnt];
        [_partialCell lineToPoint:waferPoints[3]];
        pnt = NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5]);
        [_partialCell lineToPoint:pnt];
        [_partialCell closePath];
        _partialColor = [NSColor orchidPink]; // corner cell
//------------------------------ Now the HD wafer ----------------------------------
        ...
        if(_edge) {
            double xl = hftof + _side/sin60;
            if(_iv == 0) {
                NSPoint pnt = NSMakePoint(_centre.x+xoff[4], _centre.y+yoff[4]);
                [_partialCell moveToPoint:pnt];
                pnt = NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5]);
                [_partialCell lineToPoint:pnt];
                pnt.x += 1.5*_side*cos60;
                ...
                NSPoint pnt = NSMakePoint(_centre.x+xoff[2], _centre.y+yoff[2]);
                [_partialCell moveToPoint:pnt];
                pnt = NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1]);
                [_partialCell lineToPoint:pnt];
                pnt.x -= 1.5*_side;
            ...
            pnt = NSMakePoint(_centre.x+xoff[5], _centre.y+yoff[5]);
            [_partialCell lineToPoint:pnt];
            pnt = NSMakePoint(_centre.x+xoff[0], _centre.y+yoff[0]);
            [_partialCell lineToPoint:pnt];
            pnt = NSMakePoint(_centre.x+xoff[1], _centre.y+yoff[1]);
            [_partialCell lineToPoint:pnt];
            pnt = NSMakePoint(_centre.x+xoff[2], _centre.y+yoff[2]);
        ...
        if(_edge) {
            double xl = hftof + 0.5*_side/sin60;
            if(_iv == 0) {
                NSPoint pnt = NSMakePoint(_centre.x+xoff[3], _centre.y+yoff[3]);
                pnt.x += 1.5*_side*cos60;
                pnt.y -= 1.5*_side*sin60;
                [_partialCell moveToPoint:pnt];
                pnt = NSMakePoint(_centre.x+xoff[3], _centre.y+yoff[3]);
                [_partialCell lineToPoint:pnt];
                pnt = NSMakePoint(_centre.x+xoff[4], _centre.y+yoff[4]);
                [_partialCell lineToPoint:pnt];
                pnt.x -= 1.5*_side*cos60;
