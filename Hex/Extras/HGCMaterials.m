...
- (void) buildDictionary {
    ...
    mat = @[@"WCu",@"Pb",@"Cu",@"inox",@"Si",@"scintillator",
            @"PCB",@"Air",@"foil",@"epoxy",@"kapton",@"Cfibre"];
    double X0[12] =   {5.122,5.612,14.36,17.35,93.66,413.1,175.0,3.E5,503.1,1.E6,1.E6,175.0};
    double lam[12] =  {119.9,182.6,155.1,166.0,457.5,770.7,484.2,7.E5,881.8,1.E6,1.E6,484.2};
    double dEdx[12] = {1.812,1.274,1.257,1.143,0.388,0.205,0.318, 0.0,0.185,0.00,0.00,0.318};
    ...
    dic = [NSMutableDictionary dictionaryWithCapacity:12];
    for (int i=0; i<12; i++) {
...
- (void) calculateEpoxy {
     ...
     */
      ...
      double dedxtimes[3]     = { 1.788, 1.742, 4.034}; // MeV cm2/g
      double X0per[3]         = { 34.24, 42.70, 92.32}; // g/cm2
      double lamper[3]        = {  90.2,  85.8,  71.0}; // g/cm2
...
*/
    ...
    epoxy[2] = 0.1*density*rdE;
    */
    epoxy[0] = 10.*[self compLenFor:frac ofX:X0per count:3]/density;
    epoxy[1] = 10.*[self compLenFor:frac ofX:lamper count:3]/density;
    epoxy[2] = 0.1*[self compdEdxFor:frac ofX:dedxtimes count:3]*density;
...
- (void) calculateKapton {
  /*
   ...
   But in fact these numbers are pretty crappy: DuPont gives density as 1.42, and chemical
   formula (C22H10N2O5) gives composition as shown here:
   https://pdg.lbl.gov/2005/AtomicNuclearProperties/substances/kapton.html
...
//    double x0 = 0.;
 //   double lam = 0.;
    ...
    x0 = 1./x0;
    lam = 1./lam;
    //NSLog(@"Kapton x0, rdE: %.3f, %.3f",x0,rdE);
     */
/*
    double cvals[3];
    [self compoundOfCount: 3 fractions: frac ofX0: X0per lamda: lamper dEdx: dedxtimes gives:cvals];

    kapton[0] = 10.*cvals[0]/density;  // NB: convert cm to mm
    kapton[1] = 10.*cvals[1]/density;
    kapton[2] = 0.1*cvals[2]*density;
*/
    kapton[0] = 10.*[self compLenFor:frac ofX:X0per count:3]/density;
    kapton[1] = 10.*[self compLenFor:frac ofX:lamper count:3]/density;
    kapton[2] = 0.1*[self compdEdxFor:frac ofX:dedxtimes count:3]*density;
...
*/
    ...
    double values[3];
    values[0] = 10.*[self compLenFor:frac ofX:X0per count:3]/density;
    values[1] = 10.*[self compLenFor:frac ofX:lamper count:3]/density;
    values[2] = 0.1*[self compdEdxFor:frac ofX:dedxtimes count:3]*density;
...
- (double) compLenFor: (double *) wfrac ofX: (double *) X count: (int) n {
    
    double xx = 0.;
    for (int i=0;i<n;i++) { xx  += wfrac[i]/X[i];}
    xx = 1./xx;
    return xx;
}

- (double) compdEdxFor: (double *) wfrac ofX: (double *) X count: (int) n {
